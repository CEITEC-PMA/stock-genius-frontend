...
import {
  imagemCandidato,
  votosPorCandidatos,
} from "@/utils/types/votosPorCandidatos.types";
...
export default function TabelaResultadoFinalCopy({
  dadosQuorum,
  ...
  for (let i = 0; i < cpfSemPontos.length; i++) {
    caminhoCompletoFoto[
      i
    ] = `${apiUrl}/fotosCandidato/${cpfSemPontos[i]}/${fotos[i]}`;
  }
          ...
          <Table sx={{ border: "1px solid black" }}>
            <TableHead>
              <TableRow sx={{ height: "85px" }}>
                ...
                >
                  <Typography>
                    Total de Votos (Pais, Alunos e Funcionários)
                  </Typography>{" "}
                </TableCell>
                <TableCell
                  sx={{
            ...
            <TableBody>
              {ResultadoFinalEleicao?.map((resultado, i) => (
                <TableRow key={i}>
                  ...
                  >
                    {resultado.totalVotosAlunosResponsaveisFuncionarios}
                  </TableCell>
                  <TableCell
                    sx={{
                      border: "1px solid black",
