...
export default function TabelaQuorumVotosPaisRespAlunosCopy({ dadosQuorum }: ITAbelaQuorumVotosPaisRespAlunos) {
    ...
    const qtdAlunosResponsaveis = dadosQuorum.map((quorum) => {
        return (
            quorum.totalVotosAlunosResponsaveis
        )
    })

    const somaQtdAlunosResponsaveis = qtdAlunosResponsaveis.reduce((acumulator, value) => (
        acumulator + value
    ), 0)
                ...
                <Table sx={{ border: '1px solid black' }}>
                    ...
                    <TableBody>
                        {dadosQuorum.map((dados) => (
                            <TableRow>
                                <TableCell sx={{ border: '1px solid black', width: '20%', textAlign: 'center' }}><Typography>{dados.nome}</Typography></TableCell>
                                <TableCell sx={{ border: '1px solid black', width: '20%', textAlign: 'center' }}>{dados.votosAlunos}</TableCell>
                                <TableCell sx={{ border: '1px solid black', width: '20%', textAlign: 'center' }}>{dados.votosResponsaveisVotantes}</TableCell>
                                <TableCell sx={{ border: '1px solid black', width: '20%', textAlign: 'center' }}>{dados.votosResponsaveisNaoVotantes}</TableCell>
                                <TableCell sx={{ border: '1px solid black', width: '20%', textAlign: 'center' }}>{dados.totalVotosAlunosResponsaveis}</TableCell>
                        ...
                        <TableRow>
                            <TableCell colSpan={5} sx={{ border: '1px solid black', textAlign: 'center' }}><Typography>TOTAL VOTOS VÁLIDOS SEGMENTO PAIS / RESPONSÁVEIS E ALUNOS: {somaQtdAlunosResponsaveis}</Typography></TableCell>
                        </TableRow>
